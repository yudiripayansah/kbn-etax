...
<script>
    $(document).ready(function() {
		 ...
		 Pace.track(function(){  
		   $('#tabledata').DataTable({
			...
			"processing"	: true,
			"ajax"			: {
								 "url"  		: "<?php echo site_url('ppn_wapu/load_pembetulan'); ?>",
								 "type" 		: "POST",
								 "data"			: function ( d ) {
	...
	$("#btnOpen").click(function(){	
		...
		if (vfCabang==''){
			$("#error5").html('<i style="color:#dd4b39; font-size:12px;">Cabang ke belum diisi!</i>');
			$("#dfjenisPajak").addClass("has-error");
			return false;
		}
			...
			callback: function (result) {
				if(result) {					
					$.ajax({
						url		: "<?php echo site_url('ppn_wapu/save_pembetulan') ?>",
						type	: "POST",
						...
						success	: function(result){
							if (result==1) {
								$("#modal-wp").modal("hide");
								 ...
								 $("body").removeClass("loading");
								 flashnotif('Error','Data '+vfnamapajak+' Bulan '+vfnmbulan+' Tahun '+vftahun+' Belum di Tutup!','error' );
							} else if (result==3){
								 $("body").removeClass("loading");
	...
	$("#btnDelete").click(function(){
		  bootbox.confirm({
			...
			callback: function (result) {
				if(result) {
					$.ajax({
						url		: "<?php echo site_url('ppn_wapu/delete_pembetulan') ?>",
						type	: "POST",
