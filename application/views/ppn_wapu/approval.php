...
<script>
    $(document).ready(function() {
		...
		Pace.track(function(){  
		   $('#tabledata').DataTable({			
			...
			"processing"	: true,
			"ajax"			: {
								 "url"  		: "<?php echo site_url('ppn_wapu/load_approv'); ?>",
								 "type" 		: "POST",								
								 "data"			: function ( d ) {
		...
		$("#btnApprov").click(function(){
			...
			callback: function (result) {
				if(result) {
					...
					$.ajax({
						url		: "<?php echo site_url('ppn_wapu/save_approv') ?>",
						type	: "POST",
			...
			success	: function(result){
				if (result.isSuccess==1) {	
					if(result.status_period=="OPEN"){ 
						if(result.status=="SUBMIT"||result.status=="REJECT BY PUSAT"){
	...
	{		
		...
		$.ajax({
			...
			type	: "POST",
			dataType:"json", 
			data	: ({ _searchBulan : $('#bulan').val(), _searchTahun : $('#tahun').val(), _searchPembetulan: $('#pembetulanKe').val(), _searchTipe : "APPROV" }),
			success	: function(result){						
