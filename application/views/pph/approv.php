    ...
    $(document).ready(function() {
		...
		Pace.track(function(){  
		   $('#tabledata').DataTable({			
			...
			"processing"	: true,
			"ajax"			: {
								 "url"  		: "<?php echo site_url('pph/load_approv'); ?>",
								 "type" 		: "POST",								
								 "data"			: function ( d ) {
		...
		$('.dataTables_filter input[type="search"]').attr('placeholder','Cari No Faktur/ Invoice/ Nama WP ...').css({'width':'230px','display':'inline-block'}).addClass('form-control input-sm');		
		...
		$("#btnApprov").click(function(){
			...
			callback: function (result) {
				if(result) {
					...
					$.ajax({
						url		: "<?php echo site_url('pph/save_approv') ?>",
						type	: "POST",
