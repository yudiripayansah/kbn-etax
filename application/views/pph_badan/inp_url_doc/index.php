				...
				<div class="col-lg-3">
					<div class="form-group">
						...
						<select class="form-control" id="namaPajak" name="namaPajak">
							...
							?>
							<option value="<?php echo $nama_pajak_cust ?>" data-name="<?php echo $nama_pajak_cust ?>"<?php echo $disabled ?>><?php echo $nama_pajak_cust ?></option>
							<?php 	} ?>
			  ...
			  <div class="col-lg-6 ">
				<div class="form-group">
					...
					<div class="input-group">
						<input class="form-control" id="inpnamapajak" name="inpnamapajak" placeholder="Nama Pajak" type="text" readonly>
						<span class="input-group-btn">
						<button type="button" id="getnamapajak" class="btn waves-effect waves-light btn-danger" data-toggle="modal" data-target="#modal-namapajak" ><i class="fa fa-search"></i></button>
						</span> 
					</div>
...
// call LOV page utk Nama Pajak
	$("#getnamapajak").on("click", function(){
			vnama	        = "";							
			...
			if ( ! $.fn.DataTable.isDataTable( '#tabledata-namapajak' ) ) {
				$('#tabledata-namapajak').DataTable({
					...
					"processing"	: true,
					"ajax"			: {
										 "url"  		: "<?php echo site_url('pph_badan/load_lov_nama_pajak'); ?>",
										 "type" 		: "POST"
									  },
					 "language"		: {
							"emptyTable"	: "Data not found!",	
				...
				$("#modal-namapajak input[type=search]").addClear();		
				$('#modal-namapajak .dataTables_filter input[type="search"]').attr('placeholder','Search Nama...').css({'width':'230px','display':'inline-block'}).addClass('form-control input-sm');		
		...
		{
			$("#inpnamapajak").val(vnama);
			$("#modal-namapajak").modal("hide");
		}	
...
// call save tambah data
	$("#btnSave").click(function(){				
		$.ajax({
			url		: "<?php echo site_url('pph_badan/save_url_doc') ?>",
			type	: "POST",
	...
	$("#btnDelete").click(function(){
		  bootbox.confirm({
			...
			callback: function (result) {
				if(result) {
					$.ajax({
						url		: "<?php echo site_url('pph_badan/delete_url_doc') ?>",
						type	: "POST",
						data	: $('#form-wp').serialize(),
						beforeSend	: function(){
							 $("body").addClass("loading");					
