...
$(document).ready(function() {
	...
	function eksport(x){		
		...
		if (x==1){
			var vjdl = "Eksport ";
			var url 	="<?php echo site_url(); ?>pph_badan/eksport_bl_csv";
		} else {
			var vjdl = "Eksport ";
			var url 	="<?php echo site_url(); ?>pph_badan/eksport_bl_excel";
		}
		
		bootbox.confirm({
			title: vjdl+"Daftar Rincian <span class='label label-danger'> "+vbln+" "+t+" "+vcb+"</span> ?",
			...
			callback: function (result) {
				...
				//============					
					$.ajax({			
						url		: '<?php echo site_url() ?>pph_badan/cek_data_bl_excel'+'?ledger='+ledger+'&month='+b+'&year='+t+'&cab='+c+'&akun='+vakun,				
						success	: function(result){
							if (result==1) {	
								window.open(url+'?ledger='+ledger+'&month='+b+'&year='+t+'&cab='+c+'&akun='+vakun, '_blank');
								window.focus(); 														
							} else {
